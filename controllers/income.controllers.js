...
try {
        ...
        if (!req.cookies?.accessToken) {
          ...
          const {name} = await IncomeCategory.findOne({_id:incomeCategoryId,userId:decodedToken._id});
          res.status(200).json({
            "success":true,
            categoryName:name,
          })
...
export { addIncomeCategory, getIncomeCategories ,addIncome,getAllIncomes,getIncomeCategoryNameById};
